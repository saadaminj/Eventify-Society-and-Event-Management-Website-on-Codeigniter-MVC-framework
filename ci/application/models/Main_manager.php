...
class main_manager extends CI_Model {
    ...
    public function select_all_but_id($table,$id2)
    {
        $query=$this->db->query('SELECT * from events where status=1 and id not in ( SELECT id from events where id in ( select eid from participant where par_id="'.$id2.'") or id in (Select eid from par_group where leader_id="'.$id2.'" or group_id in (select group_id from group_members where member_id="'.$id2.'")) )');
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return 0;
    ...
    public function select_event_by_participants($id2) {
        $query = $this->db->query('SELECT * from events where status=1 and  id in ( select eid from participant where par_id="'.$id2.'") or id in (Select eid from par_group where leader_id="'.$id2.'" or group_id in (select group_id from group_members where member_id="'.$id2.'"))');
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return 0;
    ...
    public function select_other_by_id($other,$id,$table) {
        $query = $this->db->query('SELECT '.$other.' FROM '.$table.' WHERE id='.$id);
        if ($query->num_rows() > 0) {
            return $query->result_array();
        } else {
            return 0;
