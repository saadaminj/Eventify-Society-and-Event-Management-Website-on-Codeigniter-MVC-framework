...
class Social_media extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId,46);
            if($checkPermission == 1 || $adminId == 1){
                $final_data['final_data'] = $this->main_manager->select_all('social_media');
                $final_data['final_data'] = $this->joins_manager->getSocialMedia();

                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/view_socialmedia',$final_data);
    ...
    public function edit($id = NULL) {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $adminId = $this->session->userdata('admin_id');
            $data['users']    = $this->main_manager->select_by_id($adminId,"users");
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId,47);
            if($checkPermission == 1 || $adminId == 1){
                $final_data = array();
                ...
                if ($this->input->post('doAct') == strtolower($this->pageAction)) {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $this->main_manager->update($id, $data, "social_media");
                        $this->session->set_flashdata('msg', 'The data has been updated successfully');
                        redirect(SITE_ADMIN_URL . "social_media/index/");
                    }##end else validation
                }##end if the form is submitted
                $final_data["final_data"] = $this->main_manager->select_by_id($id, "social_media");
                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/edit_social',$final_data);
                $this->load->view('admin/footer');
            }else{
                $this->load->view('admin/notallowed');
            }
