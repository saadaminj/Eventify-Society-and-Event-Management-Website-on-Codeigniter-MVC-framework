...
class Development_process extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId,10);
            if($checkPermission == 1 || $adminId == 1){
                $final_data['final_data'] = $this->joins_manager->getDevelopmentProcess();
                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/view_development_process',$final_data);
    ...
    public function edit($id = NULL) {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $adminId = $this->session->userdata('admin_id');
            $data['users']    = $this->main_manager->select_by_id($adminId,"users");
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId,11);
            if($checkPermission == 1 || $adminId == 1){
                $final_data = array();
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        $final_data['error'] = " ";
                         ...
                         if (!empty($_FILES["userfile"]["tmp_name"])) {
                            $config['upload_path'] = './assets/files/development_process/';
                        ...
                        $this->main_manager->update($id, $data, "development_process");
                        $this->session->set_flashdata('msg', 'The data has been updated successfully');
                        redirect(SITE_ADMIN_URL . "development_process/index");
                    }##end else validation
                }##end if the form is submitted

                $final_data["final_data"] = $this->main_manager->select_by_id($id, "development_process");
                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/edit_development_process',$final_data);
                $this->load->view('admin/footer');
            }else{
                $this->load->view('admin/notallowed');
            }
