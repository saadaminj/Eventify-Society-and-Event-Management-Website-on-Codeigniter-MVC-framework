...
class Terms_conditions extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId, 50);
            if ($checkPermission == 1 || $adminId == 1) {
                $final_data['final_data'] = $this->main_manager->select_all('terms_conditions');
                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/view_terms_conditions', $final_data);
    ...
    public function edit($id = NULL)
    {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $adminId = $this->session->userdata('admin_id');
            $data['users']    = $this->main_manager->select_by_id($adminId,"users");
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId, 51);
            if ($checkPermission == 1 || $adminId == 1) {
                $final_data = array();
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        $final_data['error'] = " ";
                         ...
                         if (!empty($_FILES["userfile"]["tmp_name"])) {
                            $config['upload_path'] = './assets/files/terms_conditions/';
                        ...
                        $this->main_manager->update(1, $data, "terms_conditions");
                        $this->session->set_flashdata('msg', 'The data has been updated successfully');
                        redirect(SITE_ADMIN_URL . "terms_conditions/index");
                    }##end else validation
                }##end if the form is submitted
                $final_data["final_data"] = $this->main_manager->select_by_id(1, "terms_conditions");
                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/edit_terms_conditions', $final_data);
                $this->load->view('admin/footer');
            } else {
                $this->load->view('admin/notallowed');
            }
