...
class Index_about extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId,8);
            if($checkPermission == 1 || $adminId == 1){
                $final_data['final_data'] = $this->joins_manager->getIndexAbout();
                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/view_index_about',$final_data);
    ...
    public function edit($id = NULL) {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $adminId = $this->session->userdata('admin_id');
            $data['users']    = $this->main_manager->select_by_id($adminId,"users");
            $checkPermission = $this->joins_manager->userPermisionByMethod($adminId,9);
            if($checkPermission == 1 || $adminId == 1){
                $final_data = array();
                ...
                if ($this->input->post('doAct') == strtolower($this->pageAction)) {
                    ...
                    $this->form_validation->set_rules('image_title', 'alt Title', 'required');

                    if ($this->form_validation->run() == FALSE) {
                        $final_data['error'] = " ";
                    } else {
                         if (!empty($_FILES["userfile"]["tmp_name"])) {
                            $config['upload_path'] = './assets/files/about/';
                        ...
                        $this->main_manager->update($id, $data, "index_about");
                        $this->session->set_flashdata('msg', 'The data has been updated successfully');
                        redirect(SITE_ADMIN_URL . "index_about/index");
                    }##end else validation
                }##end if the form is submitted

                $final_data["final_data"] = $this->main_manager->select_by_id($id, "index_about");
                $this->load->view('admin/header');
                $this->load->view('admin/sidebar',$data);
                $this->load->view('admin/edit_index_about',$final_data);
                $this->load->view('admin/footer');
            }else{
                $this->load->view('admin/notallowed');
            }
