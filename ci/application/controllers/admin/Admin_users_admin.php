...
class Admin_users_admin extends CI_Controller {
    ...
    public function admin_users($id = NULL) {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $final_data['final_data'] = $this->joins_manager->getAllUsers();
    ...
    public function event_management($id = NULL) {
        if ($this->session->userdata('adminLogged') != 'yes') {
            $this->load->view('admin/login');
        } else {
            $adminId                = $this->session->userdata('admin_id');
            $data['users'] = $this->main_manager->select_by_id($adminId,"users");
            $final_data['final_data'] = $this->joins_manager->getAllEvents();
            $this->load->view('admin/header');
            $this->load->view('admin/sidebar',$data);
            $this->load->view('admin/view_admin_events',$final_data);
            $this->load->view('admin/footer');
        }
    }
    public function acitve_inactive_event($id = NULL, $is_active = NULL) {
        if ($this->session->userdata('adminLogged') != 'yes') {
            redirect(SITE_ADMIN_URL);
        } else {
            $data = array(
            ...
            if ($this->main_manager->update($id, $data, "events") == false) {
                $error['error'] = 'Cannot save. Please try again.';
            } else {
                $this->session->set_flashdata('msg', 'The data has been updated successfully');
                redirect(SITE_ADMIN_URL . "admin_users_admin/event_management", 'refresh');
                die();
            }
        }
    }
    ...
    public function editRole($id = NULL)
    {
        if ($this->session->userdata('adminLogged') != 'yes') {
            ...
            $this->load->view('admin/header');
            $this->load->view('admin/changerole');
            $this->load->view('admin/footer');
        }        
    }
    public function success_role_change()
    {
